...
describe('useShake', () => {
    ...
    it('should detect shakes and trigger callback', () => {
        ...
        const { result } = renderHook(() => useShake(onShake, { threshold: 10, requiredShakes: 2, timeout: 1000 }));
    ...
    it('should reset count if timeout expires', () => {
        ...
        const { result } = renderHook(() => useShake(onShake, { threshold: 10, requiredShakes: 2, timeout: 500 }));

        act(() => { result.current.setIsEnabled(true); });
        const handler = addEventListenerSpy.mock.calls.find(call => call[0] === 'devicemotion')![1];

        // First Shake
        act(() => { triggerMotion(handler, 0, 0, 9.8); });
        vi.advanceTimersByTime(110);
        act(() => { triggerMotion(handler, 20, 20, 20); });
        expect(onShake).not.toHaveBeenCalled();
