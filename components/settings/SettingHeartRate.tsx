      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {t(
              'hrDesc',
              'Alerts will trigger if heart rate exceeds this value while not in workout mode.'
            )}
          </p>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-gray-600">
          ...
          <button
            ...
            className={`w-12 h-6 rounded-full transition-colors relative ${
              workoutMode ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'
            }`}
            aria-label={workoutMode ? 'Disable Workout Mode' : 'Enable Workout Mode'}
          >
            <div
              className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform ${
                workoutMode ? 'translate-x-6' : 'translate-x-0'
              }`}
            />
          </button>
        </div>
