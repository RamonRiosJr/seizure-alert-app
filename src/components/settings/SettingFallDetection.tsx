    ...
    <>
      <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl space-y-4">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300">
          {t(
            'settingsFallDesc',
            'Uses device sensors to detect high-impact falls followed by stillness.'
          )}
        </p>
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`w-12 h-6 rounded-full transition-colors relative ${
              isEnabled ? 'bg-orange-500' : 'bg-gray-300 dark:bg-gray-600'
            }`}
              ...
              <div className="grid grid-cols-3 gap-2">
                {(['low', 'medium', 'high'] as const).map((level) => (
                  <button
                    ...
                    className={`py-1 px-2 rounded-lg text-xs font-medium capitalize border ${
                      sensitivity === level
      ...
      {/* Test Mode Modal */}
      {showTestMode && <FallDetectionTestMode onClose={() => setShowTestMode(false)} />}
    </>
  );
};
